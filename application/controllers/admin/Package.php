...
class Package extends CI_Controller {
	...
	public function getPackages() {
		$data = array();
		$data['title'] = "Packages || Sunset Bali Adventure";
        ...
        $total_records = $this->m_package->getCount();
     
        if ($total_records > 0) {
            // get current page records
            $data["packages"] = $this->m_package->getCurrentPageRecordPackages($limit_per_page, $page*$limit_per_page);
                 
            $config['base_url'] = base_url() . 'admin/package';
	...
	public function getPackage() {
		try {
			...
			$result = [
				"status" => true,
				"data" => $this->m_package->getPackage($where)[0],
			...
			$result = [
				...
				"errors" => array(
					"get package"=>"Failed get package"
				)
			];
		}
